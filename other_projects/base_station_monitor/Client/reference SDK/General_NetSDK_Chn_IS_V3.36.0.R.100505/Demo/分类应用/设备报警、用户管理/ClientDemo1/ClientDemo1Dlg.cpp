...
BOOL CALLBACK MessCallBack(LONG lCommand, LONG lLoginID, char *pBuf,
...
BOOL CClientDemo1Dlg::ReceiveMessageEx(LONG lLoginID, LONG lCommand, char *pchDVRIP, LONG nDVRPort, char *pBuf, DWORD dwBufLen)
{
	BOOL bRet = FALSE;
	BOOL bIsTimeOut = m_cs.Lock();
		...
		case DH_ALARM_DECODER_ALARM_EX: //0x210E	//Alarm decoder alarm 
			{
				...
				memcpy(&alarmAlarm.m_stuAlarmDecoderAlarm, pBuf, dwBufLen);
				bRet = TRUE;
			}
		default:
			break;
...
void CClientDemo1Dlg::DHCommAlarm(char *pBuf, DWORD dwBufLen)
{
	...
	for(i=0;i<alarmInfo.m_nAlarmInputCount;i++)
	{
		if(1 == pState->alarm[i])
		{
			WORD wAlarmChannel = (WORD)pow(2,i);
...
void CClientDemo1Dlg::OnTimer(UINT nIDEvent) 
{
			...
			if(!(m_ListAlarmInfoEx.IsEmpty()))
			{
				...
				for(int i=0;i<m_ListAlarmInfoEx.GetCount();i++)
				{
					...
					case DH_ALARM_ALARMEX://External alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_10);
						break;
					case DH_MOTIONDETECT_ALARMEX://Motion detection alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_20);
						break;
					case DH_VEDIOSHELTER_ALARMEX://Video loss alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_30);
						break;
					case DH_SHELTER_ALARMEX://Camera masking alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_00);
						break;
					case DH_ENCODER_ALARMEX://encoder alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_60);
						break;	
					case DH_URGENCY_ALARMEX://Emergency alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_40);
						break;
					case DH_WIRELESS_ALARMEX://Wireless alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_50);
						break;
					...
					case DH_DISKFULL_ALARMEX://HDD full alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_70);
						break;
					case DH_DISKERROR_ALARMEX://HDD malfunction alarm 
						m_pTabNewAlarmDlg->SetAlarmInfo(alarmInfoEx.m_dwAlarm,IDC_ARRAY_80);
						break;
