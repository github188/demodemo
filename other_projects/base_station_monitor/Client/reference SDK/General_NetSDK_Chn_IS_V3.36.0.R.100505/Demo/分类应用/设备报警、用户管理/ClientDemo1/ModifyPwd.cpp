...
void CModifyPwd::OnOK() 
{
	...
	char strPwd[DH_USER_PSW_LENGTH] = {0};
	GetDlgItem(IDC_EDIT_NEWPWD1)->GetWindowText(strPwd, DH_USER_PSW_LENGTH);
	memcpy(newInfo.passWord, strPwd, DH_USER_PSW_LENGTH);
	memcpy(oldInfo.name, m_usersInfo->userList[m_dwUserID].name, DH_USER_PSW_LENGTH);
	GetDlgItem(IDC_EDIT_OLDPWD)->GetWindowText(strPwd, DH_USER_PSW_LENGTH);
	memcpy(oldInfo.passWord, strPwd, DH_USER_PSW_LENGTH);
...
BOOL CModifyPwd::OnInitDialog() 
{
	...
	GetDlgItem(IDC_EDIT_USERNAME)->SetWindowText(m_usersInfo->userList[m_dwUserID].name);
