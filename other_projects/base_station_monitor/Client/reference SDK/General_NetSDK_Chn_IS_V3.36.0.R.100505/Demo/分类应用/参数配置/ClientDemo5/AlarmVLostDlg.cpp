 ...
 */
void CAlarmVLostDlg::InitDlgInfo(DHDEV_ALARM_SCHEDULE_EX *pAlarmInfo, BYTE nAlarmIn, BYTE nAlarmOut, BYTE nChannel, BOOL bNewProtocol)
{
	if(pAlarmInfo != NULL)
	{
		int i =0;
		...
		{
			m_VideoLostInfo[i] = pAlarmInfo->struVideoLost[i];
		}
...
void CAlarmVLostDlg::OnSelchangeComboChannel() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		{
			((CClientDemo5Dlg*)AfxGetMainWnd())->GetLossAlarmConfigInfo(&m_VideoLostInfo[nNum], nIndex);
		}
		
		//Display channel corresponding information
		m_chkAlarmEn.SetCheck((m_VideoLostInfo[nNum].byAlarmEn)?1:0);
		
		DWORD dwActMask = m_VideoLostInfo[nNum].struHandle.dwActionMask;
		...
		m_chkUpload.SetCheck((m_VideoLostInfo[nNum].struHandle.dwActionFlag&DH_ALARM_UPLOAD)?1:0);
		m_chkLocalIP.SetCheck((m_VideoLostInfo[nNum].struHandle.dwActionFlag&DH_ALARM_TIP)?1:0);
		m_chkLocalTour.SetCheck((m_VideoLostInfo[nNum].struHandle.dwActionFlag&DH_ALARM_TOUR)?1:0);
		m_chkSendMail.SetCheck((m_VideoLostInfo[nNum].struHandle.dwActionFlag&DH_ALARM_MAIL)?1:0);
		
		m_chkRecord.SetCheck((m_VideoLostInfo[nNum].struHandle.dwActionFlag&DH_ALARM_RECORD)?1:0);
		m_chkOutPut.SetCheck((m_VideoLostInfo[nNum].struHandle.dwActionFlag&DH_ALARM_OUT)?1:0);
		
		int i=0;
		for (i = 0; i < m_nAlarmOut; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_ALMOP1+i))->SetCheck(m_VideoLostInfo[nNum].struHandle.byRelAlarmOut[i]?1:0);
		}
		for (i = 0; i < m_nChannel; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_RECCH1+i))->SetCheck(m_VideoLostInfo[nNum].struHandle.byRecordChannel[i]?1:0);
		}
		...
		m_dwMatrixMask = (unsigned short)m_VideoLostInfo[nNum].struHandle.dwMatrix;
		m_dwAmount = m_VideoLostInfo[nNum].struHandle.bySnapshotTimes;
		m_bLog = m_VideoLostInfo[nNum].struHandle.bLog;
		m_bMatrixEn = m_VideoLostInfo[nNum].struHandle.bMatrixEn;
...
BOOL CAlarmVLostDlg::SaveAlarmChannelInfo(int nNum)
{
	...
	{
		DH_VIDEO_LOST_CFG_EX *pInfo = &(m_VideoLostInfo[nNum]);
		if(NULL != pInfo)
		{
			pInfo->byAlarmEn = m_chkAlarmEn.GetCheck();
...
BOOL CAlarmVLostDlg::SaveWeekInfo(int nNum, int nWeek)
{
	...
	if(-1 < nNum && -1 < nWeek)
	{
		if(UpdateData(TRUE))
		{
			DH_TSECT *pPeriodInfo = &(m_VideoLostInfo[nNum].stSect[nWeek][0]);
			...
			pPeriodInfo = &(m_VideoLostInfo[nNum].stSect[nWeek][1]);
			...
			pPeriodInfo = &(m_VideoLostInfo[nNum].stSect[nWeek][2]);
			...
			pPeriodInfo = &(m_VideoLostInfo[nNum].stSect[nWeek][3]);
			...
			pPeriodInfo = &(m_VideoLostInfo[nNum].stSect[nWeek][4]);
			...
			pPeriodInfo = &(m_VideoLostInfo[nNum].stSect[nWeek][5]);
...
void CAlarmVLostDlg::OnSelchangeComboAlarmweek() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		if(CB_ERR != nIndex)
		{
			...
			for(int i=0;i<6;i++)
			{
				DH_TSECT periodInfo = m_VideoLostInfo[nNum].stSect[nIndex][i];
				ShowPeriodInfo(&periodInfo,i+1);
			}
...
void CAlarmVLostDlg::ShowPeriodInfo(DH_TSECT *pTSect, BYTE nNum)
{
	switch(nNum) {
	case 1:
