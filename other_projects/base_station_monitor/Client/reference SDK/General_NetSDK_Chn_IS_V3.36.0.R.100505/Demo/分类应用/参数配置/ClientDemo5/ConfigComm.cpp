...
/*
 *	Function£ºDisplay COM information
 *	Input parameter£º
 *	pCommInfo£ºCOM configuration structure 
 *	nCommCount£ºCOm amount
 *	nChannelCount£ºChannel amount 
 */
void CConfigComm::InitDlgInfo(DHDEV_COMM_CFG_EX *pCommInfo, BYTE nCommCount, BYTE nChannelCount)
{
	if(pCommInfo != NULL)
	{
		...
		int nCount = pCommInfo->dw232FuncNameNum;
		for (i = 0; i < nCount; i++)
		{
		...
		nCount = pCommInfo->dwDecProListNum;
		for (i = 0; i < nCount; i++)
		{
...
void CConfigComm::OnSelchangeCombo485com() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		m_ctlStopBit485.SetCurSel(m_CommInfo.stDecoder[nNum].struComm.byStopBit);
...
void CConfigComm::SaveComm485Info(int nNum)
{
	if(-1 < nNum)
	{
		...
		m_CommInfo.stDecoder[nNum].struComm.byStopBit = m_ctlStopBit485.GetCurSel();
