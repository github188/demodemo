...
void CAlarmMotion::InitDlgInfo(DHDEV_ALARM_SCHEDULE_EX *pAlarmInfo, BYTE nAlarmIn, BYTE nAlarmOut, BYTE nChannel, BOOL bNewProtocol) 
{
	if(pAlarmInfo != NULL)
	{
		int i =0;
		...
		{
			m_AlarmMotionInfo[i] = pAlarmInfo->struMotion[i];
		}
...
void CAlarmMotion::OnSelchangeComboChannel() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		{
			((CClientDemo5Dlg*)AfxGetMainWnd())->GetMotionAlarmConfigInfo(&m_AlarmMotionInfo[nNum], nIndex);
		}
		
		//Display channel corresponding information
		m_ctlAlarmEn.SetCheck((m_AlarmMotionInfo[nNum].byMotionEn)?1:0);
		SetDlgItemInt(IDC_EDIT_LEVEL,m_AlarmMotionInfo[nNum].wSenseLevel);
		
		DWORD dwActMask = m_AlarmMotionInfo[nNum].struHandle.dwActionMask;
		...
		m_chkUpload.SetCheck((m_AlarmMotionInfo[nNum].struHandle.dwActionFlag&DH_ALARM_UPLOAD)?1:0);
		m_chkLocalIP.SetCheck((m_AlarmMotionInfo[nNum].struHandle.dwActionFlag&DH_ALARM_TIP)?1:0);
		m_chkLocalTour.SetCheck((m_AlarmMotionInfo[nNum].struHandle.dwActionFlag&DH_ALARM_TOUR)?1:0);
		m_chkSendMail.SetCheck((m_AlarmMotionInfo[nNum].struHandle.dwActionFlag&DH_ALARM_MAIL)?1:0);
		
		m_chkRecord.SetCheck((m_AlarmMotionInfo[nNum].struHandle.dwActionFlag&DH_ALARM_RECORD)?1:0);
		m_chkOutPut.SetCheck((m_AlarmMotionInfo[nNum].struHandle.dwActionFlag&DH_ALARM_OUT)?1:0);
		
		int i=0;
		for (i = 0; i < m_nAlarmOut; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_ALMOP1+i))->SetCheck(m_AlarmMotionInfo[nNum].struHandle.byRelAlarmOut[i]?1:0);
		}
		for (i = 0; i < m_nChannel; i++)
		{
			((CButton*)GetDlgItem(IDC_CHECK_RECCH1+i))->SetCheck(m_AlarmMotionInfo[nNum].struHandle.byRecordChannel[i]?1:0);
		}
		...
		m_dwMatrixMask = (unsigned short)m_AlarmMotionInfo[nNum].struHandle.dwMatrix;
		m_dwAmount = m_AlarmMotionInfo[nNum].struHandle.bySnapshotTimes;
		m_bLog = m_AlarmMotionInfo[nNum].struHandle.bLog;
		m_bMatrixEn = m_AlarmMotionInfo[nNum].struHandle.bMatrixEn;
...
void CAlarmMotion::OnSelchangeComboAlarmweek() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		if(CB_ERR != nIndex)
		{
			...
			for(int i=0;i<6;i++)
			{
				DH_TSECT periodInfo = m_AlarmMotionInfo[nNum].stSect[nIndex][i];
				ShowPeriodInfo(&periodInfo,i+1);
			}
...
BOOL CAlarmMotion::SaveAlarmChannelInfo(int nNum)
{
	...
	{
		DH_MOTION_DETECT_CFG_EX *pInfo = &(m_AlarmMotionInfo[nNum]);
		if(NULL != pInfo)
		{
			pInfo->byMotionEn = m_ctlAlarmEn.GetCheck();
			pInfo->wSenseLevel = GetDlgItemInt(IDC_EDIT_LEVEL);
...
BOOL CAlarmMotion::SaveWeekInfo(int nNum, int nWeek)
{
	...
	if(-1 < nNum && -1 < nWeek)
	{
		if(UpdateData(TRUE))
		{
			DH_TSECT *pPeriodInfo = &(m_AlarmMotionInfo[nNum].stSect[nWeek][0]);
			...
			pPeriodInfo = &(m_AlarmMotionInfo[nNum].stSect[nWeek][1]);
			...
			pPeriodInfo = &(m_AlarmMotionInfo[nNum].stSect[nWeek][2]);
			...
			pPeriodInfo = &(m_AlarmMotionInfo[nNum].stSect[nWeek][3]);
			...
			pPeriodInfo = &(m_AlarmMotionInfo[nNum].stSect[nWeek][4]);
			...
			pPeriodInfo = &(m_AlarmMotionInfo[nNum].stSect[nWeek][5]);
...
void CAlarmMotion::ShowPeriodInfo(DH_TSECT *pTSect, BYTE nNum)
{
	switch(nNum) {
	case 1:
...
void CAlarmMotion::OnBtnArea() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		areaDlg.SetArea((BYTE*)m_AlarmMotionInfo[nNum].byDetected, m_AlarmMotionInfo[nNum].wMotionRow, 
								m_AlarmMotionInfo[nNum].wMotionCol);
		if (IDOK == areaDlg.DoModal())
		{
			memcpy(m_AlarmMotionInfo[nNum].byDetected, areaDlg.GetArea(), DH_MOTION_ROW*DH_MOTION_COL);
		}
	}
	else
	{
