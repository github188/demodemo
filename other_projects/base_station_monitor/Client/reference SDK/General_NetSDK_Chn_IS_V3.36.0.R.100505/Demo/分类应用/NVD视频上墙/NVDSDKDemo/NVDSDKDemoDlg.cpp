...
void CALLBACK cbDisConnectFunc(LONG lLoginID, char *pchDVRIP, LONG nDVRPort, DWORD dwUser) 
{
...
void CALLBACK cbHaveReConnectFunc(LONG lLoginID, char *pchDVRIP, LONG nDVRPort, DWORD dwUser)
{
	CNVDSDKDemoDlg *dlg = (CNVDSDKDemoDlg *)dwUser;
	if (dlg == NULL)
	{
...
BOOL CALLBACK cbMessCallBack(LONG lCommand, LONG lLoginID, char *pBuf, DWORD dwBufLen, char *pchDVRIP, LONG nDVRPort, DWORD dwUser)
{
	CNVDSDKDemoDlg *dlg = (CNVDSDKDemoDlg *)dwUser;
	if (dlg == NULL)
	{
...
void CNVDSDKDemoDlg::InitControl()
{
	...
	m_screenPannel.SetShowWindow(SPLIT4, SPLIT4);
...
void CNVDSDKDemoDlg::OnTimer(UINT nIDEvent) 
{
	// TODO: Add your message handler code here and/or call default
			...
			if ( nData <= 16 && nData >= 0)
			{
				...
				if (pDeviceInfo != NULL)
				{
					...
					if (selectNode != NULL)
					{
						...
						if (pDecoderInfo != NULL)
						{
							if (_stricmp(m_encoderTree.GetItemText(enselectNode), "Sub") == 0) 
							{
								nStreamType = 1;
							}
							else if(_stricmp(m_encoderTree.GetItemText(enselectNode), "Main") == 0)
							{
								nStreamType = 0;
							}
							else
							...
							strcpy(stuEncoderInfo.szDevIp, pDeviceInfo->strDevIp);
							...
							LONG lOperateHandle = CLIENT_SwitchDecTVEncoder(pDecoderInfo->lLoginID, pSwitchTvParam->nEncoderNum, 
...
void CNVDSDKDemoDlg::OnDblclkTreeDecoder(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (selectNode != NULL)
	{
		...
		if (pDecoderInfo != NULL)
		{
			...
			m_screenPannel.SetShowWindow(nMain, SPLIT4);
	...
	if (selectNode != NULL)
	{
		...
		if (pDecoderInfo != NULL)
		{
			if (!bMultiScreen)
			{
				...
				{
					bEncoderChannel[i] = nMonitorID*4+i;
				}
				...
				LONG lOperateHandle = CLIENT_CtrlDecTVScreen(pDecoderInfo->lLoginID, nMonitorID, TRUE, nSplitNum, bEncoderChannel, 4, pCtrlTVParam);
				if (lOperateHandle == 0)
				{
					delete pCtrlTVParam;
			...
			{
				// 多画面变单画面
				BYTE bEncoderChannel = nMonitorID*4 + nWndID;
				...
				LONG lOperateHandle = CLIENT_CtrlDecTVScreen(pDecoderInfo->lLoginID, nMonitorID, TRUE, 1, &bEncoderChannel, 1, pCtrlTVParam);
				if (lOperateHandle == 0)
				{
					delete pCtrlTVParam;
...
void CNVDSDKDemoDlg::OnSize(UINT nType, int cx, int cy) 
{
	...
	else
	{
		...
		decoderRect.bottom = m_clientRect.bottom - 100;
		...
		encoderRect.bottom = m_clientRect.bottom - 100;
...
void CNVDSDKDemoDlg::OnRebootDevice(WPARAM wParam, LPARAM lParam)
{
