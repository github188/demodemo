...
void CSearchTVDlg::SetDlgParam(LONG lLoginID, int nDevChannel)
{
	m_hCurLoginID = lLoginID;
	m_nDecChannel = nDevChannel;
}

void CSearchTVDlg::InsertListView(int nChannelId, int nWndId, CString strIp, int nPort, CString strUser, CString strPwd, CString strEnable, CString strWorkMode, CString strConnType, int nListenPort, CString strStreamType, CString strSnapMode)
{
...
void CSearchTVDlg::OnButChangeenable() 
{
	...
	LONG lHandle = CLIENT_SwitchDecTVEncoder(m_hCurLoginID, stuEncoderInfo.byEncoderID, &stuEncoderInfo);
	if (lHandle == 0)
	{
