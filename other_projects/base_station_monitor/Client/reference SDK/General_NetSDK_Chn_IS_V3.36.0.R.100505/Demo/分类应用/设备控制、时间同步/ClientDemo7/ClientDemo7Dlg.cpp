...
void CClientDemo7Dlg::OnButtonDiskctrl() 
{
	...
	if(0 != m_LoginID)
	{
		...
		if (m_nCurDiskType == 0) 
		{
			bSuccess = CLIENT_ControlDevice(m_LoginID,DH_CTRL_DISK,&diskParam, 5000);
		}
		else
		{
			bSuccess = CLIENT_ControlDevice(m_LoginID,DH_CTRL_SDCARD,&diskParam, 50000);
		}
...
void CClientDemo7Dlg::GetDiskInfo()
{
	if( 0 != m_LoginID)
	{
		...
		if (bSuccess) 
		{
			...
			for(i=0;i<m_comboDisk.GetCount();i++)
			{
				m_comboDisk.DeleteString(i);
			}
