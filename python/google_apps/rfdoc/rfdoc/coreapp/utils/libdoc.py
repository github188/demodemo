...
from robot.parsing.metadata import ImportSetting
...
if ROBOT_VERSION < '2.1':
    from robot.output import abstractlogger
else:
    from robot.output import loggerhelper as abstractlogger
...
def get_last_update_author(path = ''):
    if path.endswith('c'):
        path = path[:-1] 
    author = version = '' 
    ...
    try:
        out = pipe.read()
        match = re.search('Last Changed Author: (\w+)', out, re.I)
        if match is not None:
            author = match.group(1)            
#        match = re.search('Last Changed Date: ([0-9\-]+ [0-9:]+)', out, re.I)
#        if match is not None:
#            version = match.group(1)
        return author
    except:
        print "Please install svn console!"
...
class IpaHtmlReader(HtmlReader):
    def __init__(self):
        HtmlReader.__init__(self)
        self._handlers['p_end'] = self.p_end
...
class IpaTabularRawData(TabularRawData):

    def __init__(self, path, strip_comments=True, process_curdir=True):
        self._document_tag_seen = False

        class DummyLog(abstractlogger.AbstractLogger):
            
            def _write(self, message):
                print message
                    
        if ROBOT_VERSION < '2.1':
            TabularRawData.__init__(self, path, DummyLog('INFO'), strip_comments)
        else:
            TabularRawData.__init__(self, path, strip_comments)
    
    def _process_cells(self, cells):
        
        for i in range(len(cells)):
            if cells[i].count("[Document]") and i + 1 < len(cells):
                cells[i + 1] = cells[i + 1].replace("\n", "\\n")
        
        cells = TabularRawData._process_cells(self, cells)
...
class IpaDataReader:
    ...
    def read(self, path, strip_comments=True, process_curdir=True):
        datafile = self._open_datafile(path)
        rawdata = IpaTabularRawData(path, strip_comments, process_curdir)
        self._get_reader(path).read(datafile, rawdata)
        datafile.close()
        return rawdata

    def _open_datafile(self, path):
        try:
            return open(path)
        except:
...
def read_html(file):
    ...
    ret = UserLibrary(resource.user_keywords, file)
    ret.source = resource.source
...
class ResourceFile:
    ...
    def __init__(self, path):
        self.source = path
        rawdata = IpaDataReader().read(path)
...
#    return rawdata
#    doc = ""
#    for entry in rawdata.settings:
#        if entry.name.strip() == "[Document]" or entry.name.strip() == "[Documentation]":
#            doc += entry.value[0]
#    kws = []
#    for kw in rawdata.keywords:
#        kw.name = kw.name.strip()
#        if kw.name != "":
#            kwdoc = ""
#            args = []
#            for meta in kw.metadata:
#                if meta.name == "Arguments":
#                    args = meta.value
#                if meta.name == "Document" or meta.name == "Documentation":
#                    kwdoc = "".join(meta.value)
#            if kwdoc.lower().find("internal use only") < 0:
#                #kw = KeywordRes(kw.name, kwdoc, args, os.path.split(file)[1])
#                kws.append(kwdoc)
#            else:
#                print "Ignoring internal keyword '%s' in file '%s'" % (kw.name, os.path.split(file)[1])
#    return kws, doc
        
def LibraryDoc(libname, arguments=None, newname=None, type=None):
    ext = os.path.splitext(libname)[1].lower()
    if  ext in ('.html', '.htm', '.xhtml', '.tsv', '.txt', '.rst', '.rest'):
        return ResourceDoc(libname, arguments, newname, type)
    elif ext == '.xml':
        ...
        return PythonLibraryDoc(libname, arguments, newname, type)
...
class PythonLibraryDoc(_DocHelper):
    ...
    def __init__(self, name, arguments=None, newname=None, type=None):        
        ...
        if type == "TestCase Resource":  
            self.source = name          
            self.author = get_last_update_author(name)
        else:
            self.source = lib.source 
            self.author = ''
        self.display_source = self.source
        self.base_dir = os.path.dirname(os.path.dirname(self.source))
...
class ResourceDoc(PythonLibraryDoc):
    ...
    def __init__(self, name, arguments=None, newname=None, type=None):
        PythonLibraryDoc.__init__(self, name, arguments, newname, type)
        self.display_source = self.source[self.source.lower().find('testcase'):]
        
    def _import(self, path, arguments):
        if arguments:
            raise DataError("Resource file cannot take arguments.")
        return read_html(self._find_resource_file(path))
...
class KeywordDoc(_BaseKeywordDoc):
    ...
    def __init__(self, handler, library):
        ...
        self._parse_for_ipamml(handler, library)
    ...
    def _parse_args(self, handler):
        if not hasattr(handler, "args"): return ([], [], None)
    ...
    def _parse_for_ipamml(self, handler, library):
        if hasattr(handler, '_method') and handler._method:
            kw_file = os.path.abspath(get_co_filename(handler._method))
            if os.path.basename(kw_file).startswith('decorators'):
                try:
                    kw_file = os.path.abspath(handler._method._co_filename)
                except:
                    pass    
            #self.source = os.path.normpath(handler._method.func_code.co_filename)        
            self.source = kw_file
            if library.lib_type == 'IpaMml':
                if "__" in handler._method.__name__:
                    version = handler._method.__name__.split("__")[-1]
                    from robot import utils
                    keyword_name = utils.printable_name(version.replace("_", " "))
                    dut_version = version.lower() != 'cb_8_9_0' and "@{%s}" % version or ""
                    self.name = re.sub(r"%s$" % keyword_name, 
                                       dut_version, 
                                       self.name, 1).strip()          
        else:
            self.source = library.source.endswith('c') and library.source[:-1] or library.source
        if library.lib_type == 'IpaMml':
            self.display_source = self.source.lower().replace(library.base_dir.lower(), "<LIB>")
        elif library.lib_type == 'TestCase Resource':
            self.display_source = self.source[self.source.lower().find('%stestcase%s' % (os.path.sep, os.path.sep))+1:]
        else:
            self.display_source = self.source
        
        import hashlib
        pattern = r"^\s*(?:MML|STE(?: \w+)?):([^<\n]{4,30})$"
        self.args_doc = ", ".join(self.args)
        self.mml = ",".join((e.group(1).strip()
                             for e in re.finditer(pattern, self.doc, re.M)))
        
        #<library_name><documentation><args><source>
        signature = "%s,%s,%s,%s" % (library.name, self.doc, self.args_doc, self.source)
        if isinstance(signature, unicode): signature = signature.encode('utf-8')
        self.md5 = hashlib.md5(signature).hexdigest()
