...
class OP_NOT(AST_OP):
    def __init__(self):
        AST_OP.__init__(self, "not", 3, 1)
    def evalute(self, *args):
        return not self.child[0].evalute(*args)
    
factor = re.compile('([()|&!]|[^()|&!]+)')
def compile_ast_tree(expr, parser = None):
    ...
    for item in items:
        if item == "|":
            stack[-1].insert_node(OP_OR())
        elif item == "&":
            stack[-1].insert_node(OP_AND())
        elif item == "!":
            stack[-1].insert_node(OP_NOT())
        elif item == "(":
