...
class ControllableDaemon(object):
    ...
    def __init__(self, name='controllable daemon', running_flag='.running', closing_flag='.closing', daemon=True, once_only=False):
        self.name = name
    ...
    def start(self):
        ...
        while not interrupted and not self.is_closing():
            try:
                ...
                interrupted = True
            if self.once_only:
                self.logger.info('%s once only' % self.name)
                break
        self.finish()
