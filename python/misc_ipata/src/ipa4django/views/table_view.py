...
class TableView(object):
    ...
    def as_table(self):
        if not self.__as_talbe:
            ...
            output = "<!--%s -->" % self.comments
            if self.paging_controller: output += self.render_page_nav()
            
            output += "<table " + ' '.join(['%s="%s"' % (k,v) for k, v in self.attr.iteritems()]) + ">\n"
            if not self.raw_mode:
                output += self.render_header(cols)
                output += self.render_body(data, cols)
            else:
                output += self.render_raw_mode_body(data)
            output += "</table>"
    ...
    def render_header(self, cols):
        data = "<tr>"
        for e in cols:
            row_data = "<th>" + self.cells.render_title(e) + "</th>\n" 
    ...
    def render_raw_mode_body(self, data):
        
        output = ""
        data = [ e for e in data ]
        for i in range(self.data_count):
            if i % self.raw_columns == 0:
                output += "<tr>"
            
            output += "<td>"
            output += str(self.cells.raw_cell(data[i]))
            output += "</td>"

            if (i + 1) % self.raw_columns == 0:
                output += "</tr>\n"
        
        if (i + 1) % self.raw_columns != 0:
            for e in range(self.raw_columns - (i + 1) % self.raw_columns):
                output += "<td>"
                output += str(self.cells.raw_empty_cell())
                output += "</td>"
    ...
    def _render_row(self, row, cols):
        ...
        for e in cols:
            ...
            row_data = "<td%s>" % td_attr + str(getattr(self.cells, e)(row)) + "</td>\n"
