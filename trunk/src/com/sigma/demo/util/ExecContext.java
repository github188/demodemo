...
public class ExecContext {
	protected static final Logger log = LogManager.getLogger(ExecContext.class);
	...
	public int process(final Process p, Collection<ProcessFilter> filters){
		...
		ReadPipe stdOut = new ReadPipe(p.getInputStream(), stdIn, "Script STDOUT:", filters);
		ReadPipe errOut = new ReadPipe(p.getErrorStream(), stdIn, "Script STDERR:", filters);
		...
		try {
			...
			log.fatal("The demo script can't exits!:" + p.toString());
		}
	...
	class ReadPipe extends Thread{
		...
		public ReadPipe(InputStream in, PrintWriter stdIn, String logPropmt, Collection<ProcessFilter> filters){
